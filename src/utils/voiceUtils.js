 ...
 */
export const indonesianNumbers = {
 ...
 * @returns {object} An object containing the parsed expression and whether it's valid
 */
const parseCompleteExpression = (transcript) => {
  const lowerTranscript = transcript.toLowerCase();
  let mathExpression = '';
  let currentNumber = '';
  ...
  console.log('Parsed math expression:', mathExpression);
  
  // Clean up the expression to ensure it's valid
  mathExpression = mathExpression.replace(/[^0-9+\-*/().^xπe√∛sincostalogexpabspi]/g, '');
  
  return {
    expression: mathExpression,
    valid: mathExpression.length > 0 && foundOperation
  };
};
